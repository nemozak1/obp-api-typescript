/* tslint:disable */
/* eslint-disable */
/**
 * Open Bank Project API
 * An Open Source API for Banks. (c) TESOBE GmbH. 2011 - 2025. Licensed under the AGPL and commercial licences.
 *
 * OpenAPI spec version: v5.1.0
 * Contact: contact@tesobe.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

import { Obpv310managementbanksBANKIDaccountsACCOUNTIDAccountRoutings } from './obpv310managementbanks-bankidaccounts-accountidaccount-routings';
import { Obpv510consumervrpconsentrequestsFromAccountBranchRouting } from './obpv510consumervrpconsentrequests-from-account-branch-routing';
 /**
 * 
 *
 * @export
 * @interface InlineResponse20024Account
 */
export interface InlineResponse20024Account {

    /**
     * @type {string}
     * @memberof InlineResponse20024Account
     * @example gh.29.uk
     */
    bank_id: string;

    /**
     * @type {string}
     * @memberof InlineResponse20024Account
     * @example 8ca8a7e4-6d02-40e3-a129-0b2bf89de9f0
     */
    account_id: string;

    /**
     * @type {string}
     * @memberof InlineResponse20024Account
     * @example 330
     */
    account_type: string;

    /**
     * @type {Array<Obpv310managementbanksBANKIDaccountsACCOUNTIDAccountRoutings>}
     * @memberof InlineResponse20024Account
     */
    account_routings: Array<Obpv310managementbanksBANKIDaccountsACCOUNTIDAccountRoutings>;

    /**
     * @type {Array<Obpv510consumervrpconsentrequestsFromAccountBranchRouting>}
     * @memberof InlineResponse20024Account
     */
    branch_routings: Array<Obpv510consumervrpconsentrequestsFromAccountBranchRouting>;
}
